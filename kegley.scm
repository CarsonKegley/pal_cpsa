(herald "NSL + message" 
    (comment "Kegley Edition")
)

(defprotocol kegley basic ;;Basic enjoyer
    (defrole initiator 
    (vars (nonce_a nonce_b message data) (name_a name_b name))
    (trace
        (send (enc nonce_a name_a (pubk name_b)))
        (recv (enc nonce_a nonce_b name_b (pubk name_a)))
        (send (enc nonce_b message (pubk name_b)))
    ))
    ...
    (trace 
        ...
        (recv (enc nonce_b message (pubk name_b)))
    ))
)
